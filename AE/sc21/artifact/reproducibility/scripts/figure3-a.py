...
def plot3_average(benchmarks,core_freq):
    name=[]
    name_jpi=[]
    name_dev=[]
    min_jpi=9999;
    ...
    for benc in benchmarks:
        ...
        jpi_list_core,tipi_list_core,deviation_core=plot2_average_core_uncore(benc,core_freq,1);
        ...
        #added by akshat
        if (len(jpi_list_core[0])==len(jpi_list_core[1])) and (len(jpi_list_core[1])==len(jpi_list_core[2])):
            pass
        else:
        ...
        for num_tipi in range(len(jpi_list_core[0])):
            name.append( benc+" ("+   str(round(tipi_list_core[0][num_tipi]*TIPI_slab,3))+"-"+str(round((tipi_list_core[0][num_tipi]+1)*TIPI_slab,3))   +")"  )
            jpi_freq=[]
            jpi_freq_Dev=[]
            for freq_ind in range(len(core_freq)):
                jpi_freq.append(jpi_list_core[freq_ind][num_tipi]);
                jpi_freq_Dev.append(deviation_core[freq_ind][num_tipi]);
                min_jpi=min(min_jpi,jpi_list_core[freq_ind][num_tipi]);
                max_jpi=max(max_jpi,jpi_list_core[freq_ind][num_tipi]);
            name_jpi.append(jpi_freq);
            name_dev.append(jpi_freq_Dev);
    ...
    for freq_ind in range(3):
        arr_jpi=[]
        arr_dev=[]
        for nam in range(len(name)):
            arr_jpi.append(name_jpi[nam][freq_ind]);
            arr_dev.append(name_dev[nam][freq_ind]);
            label_freq=""
            if(freq_ind==0):
                label_freq="CFmin"
            elif(freq_ind==1):
                ...
                label_freq="CFmax"
        plt.bar(np.arange(len(name))+width*freq_ind,arr_jpi, yerr=arr_dev,color = color[freq_ind],width = width,label=label_freq);

    plt.xlabel("Tor Inserts per Instruction (TIPI)")
    plt.ylabel('Joules Per 10'+get_super('9') +' Instructions (JPI)')
    plt.ylim(0,int(max_jpi+1))
    plt.grid(axis = 'y')
    plt.xticks(np.arange(len(name)) + width,name,rotation=15,horizontalalignment='center');
    plt.legend(loc=1,fontsize=8)
    plt.savefig("figure3-a.png",dpi=300, bbox_inches='tight')
    plt.close()


core_freq=["C12","C18","C23"]
#benchmarks=['uts','sorIR','heatIR','miniFE','HPCCG','amg'];
benchmarks=['uts','heatIR','miniFE','HPCCG','sorIR','amg'];
n = len(sys.argv)
if (n==1):
    data_map=read_core(0);
    benchmarks1=[];
    for k in data_map:
        benchmarks1.append(k);
    if(len(benchmarks1)!=len(benchmarks)):
        benchmarks=benchmarks1;
    plot3_average(benchmarks,core_freq);
elif (n==2):
    benc=benchmarks[int(sys.argv[1])]
    plot3_average([benc],core_freq);
